...
class Action(reactor.Base):
    ...
    def load_config(self):
        ...
        if not path:
            self.ABORT("No Reactor Actions configured, missing {}actions.json"
...
#        print(response)
    ...
    def _do__roll_package(self, target, action):
        ...
        try:
            name = "deploy_stage." + \
                   self.env.get("REACTOR_PRODUCT", 'NO_REACTOR_PRODUCT') + "_" + \
                   self.env.get("REACTOR_MODULE", 'NO_REACTOR_MODULE')
    ...
    def _do__cmd(self, target, action, exc, env=dict(), echo=False):
        ...
        if env=False, do not use the reactor environ settings, just the default os
        """
    ...
    def do(self, target, export_meta=False):
        ...
        self.env['REACTOR_ACTION_BASE'] = self.action_fqpath
    ...
    def db_connect(self):
        """
        Open a connection to the back-end reactor-core
        """
        if self.dbo:
            return
        self.dbo = Core(base=self)
