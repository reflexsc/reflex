...
import copy
import reactor
from reactor import json4store
import dictlib
...
################################################################################
class VerboseBase(reactor.Base):
    verbose = True
...
class ConfigProcessor(VerboseBase):
    ...
    ############################################################################
    def __init__(self, verbose=True, base=None, core=None):
        super(ConfigProcessor, self).__init__(verbose=verbose)
        if not base: # sorry, we need reactor.Base
            raise ValueError("Missing Reactor Base (base=)")
        if not core: # and core
            raise ValueError("Missing Reactor Core (core=)")
        reactor.Base.__inherit__(self, base)
        self.core = core # allows for overriding during testing
    ...
    def macro_expand_dict(self, dictionary):
        ...
        >>> base = reactor.Base()
        >>> test = ConfigProcessor(base=base, core=base)
    ...
    def flatten(self, name, exported=False):
        conf = Config(name, base=self, core=self.core, verbose=self.verbose).conf
    ...
    def _flatten(self, conf, name, exported=False, toplevel=False):
        ...
        new = Config(name, base=self, core=self.core, verbose=self.verbose).load()
        ...
        if conf.imports:
            for iname in conf.imports:
                ...
                iconf = Config(iname, base=self, core=self.core, verbose=self.verbose).load()
...
class Config(VerboseBase):
    ...
    ############################################################################
    def __init__(self, name, base=None, core=None, verbose=True):
        super(Config, self).__init__(verbose=verbose)
        if not base: # we need reactor.Base
            raise ValueError("Missing Reactor Base (base=)")
        reactor.Base.__inherit__(self, base)
        if not core: # and a reactor.Core() object
            raise ValueError("Missing Reactor Core (core=)")
    ...
    def load(self):
        obj = self.core.get_object('config', self.conf.name)
