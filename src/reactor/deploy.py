...
class Deploy(reactor.Base):
    ...
    ############################################################################
    def core_get(self, otype, oname):
        """helper function"""
        return self.core.cache_get_object(otype, oname, notify=False)

    def core_put(self, otype, oname, obj):
        """helper function"""
        return self.core.cache_update_object(otype, oname, obj, notify=False)
    ...
    def target_cli(self, service, args):
        ...
        """
        obj = self.core_get("service", service)
        if args.version:
            # FUTURE: Check for valid Build object first
            ver = args.version.strip()

            obj['target'] = ver
            self.core_put("service", service, obj)

            # verify and bypass cache
            obj = self.core_get("service", service) # bypass cache
    ...
    def status_cli(self, service, args):
        ...
        for node_name in nodes:
            node = self.core_get("instance", node_name)
...
#        - query Core to identify target local service should be at
