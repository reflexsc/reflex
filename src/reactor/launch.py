...
import dictlib
import reactor
from reactor.config import ConfigProcessor
from reactor import json4human, json4store
from reactor.backend import Core
from reactor.action import Action

################################################################################
# pylint: disable=too-many-instance-attributes
class App(reactor.Base):
    ...
    def launch_service_prep(self, name, commit=True):
        ...
        for key in ('REACTOR_APIKEY', 'REACTOR_TOKEN', 'REACTOR_SECRET'):
    ...
    def _load_reactor_core_config(self, target, commit=False):
        """get a from Reactor Core"""
        cproc = ConfigProcessor(base=self, core=self.dbo)
    ...
    def _launch_prep_action(self, action_target, commit=True):
        ...
        over remotely defined (Reactor Core) configs.
        ...
        # Load Reactor Core config, after ^^ environ changes
        self._load_reactor_core_config(self.launch_service['config'], commit=commit)

        # load action configs.  pull Reactor Core config expanded, merge with action config
    ...
    def _launch_prep_exec(self, name, commit=True):
        ...
        # APP_RUN_DIR is legacy -- remove once everybody is cutover
        os.environ["APP_RUN_DIR"] = self.launch_rundir
        ...
        # ylint: disable=line-too-long
        self._load_reactor_core_config(self.launch_service['config'], commit=commit)
        #cproc = ConfigProcessor(base=self, core=self.dbo) #dest=self.launch_cfgdir, commit=commit)
        #conf = cproc.flatten(self.launch_service['config'])
        #if commit:
        #    cproc.commit(conf, dest=self.launch_cfgdir)
        #self.launch_config = conf
#        self.launch_config.flatten(self.launch_service['config'], commit=commit)
#        self.launch_config = Config(base=self, dest=self.launch_cfgdir, commit=commit)
#        self.launch_config.flatten(self.launch_service['config'], commit=commit)
...
class LaunchCli(App):
    ...
    def get_target(self, *argv):
        ...
        """
        env_service = os.environ.get("REACTOR_SERVICE")
        if argv and len(argv[0]):
            return argv[0]
        elif not env_service:
            self.ABORT("No target defined (either as argument or set as REACTOR_SERVICE)")
        else:
            return env_service
