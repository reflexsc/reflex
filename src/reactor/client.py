...
Wrapper handling Reactor Apikey
...
import jwt
import reactor
from reactor import json2data, json4human, json4store
...
################################################################################
class Session(reactor.Base):
    """Session object, which logs into Reactor Core and manages the session"""
    session_jti = None
    session_sid = None
    session_secret = None
    session_expires = 0
    ...
    def _login(self, force=False):
        ...
        if not self.apikey_name:
            name, b64_secret = (self.cfg['REACTOR_APIKEY'] + ".").split(".")[0:2]
        ...
        result = requests.get(self.cfg['REACTOR_URL'] + "/token", headers={
            "X-Apikey": key_jwt,
    ...
    def _call(self, func, target, *args, **kwargs):
        """Call Reactor Core, wrapped with authentication and session management"""
        self._login()
        ...
        target = self.cfg['REACTOR_URL'] + "/" + target
