...
import dictlib
import reactor
from reactor.backend import CoreCli, Core
from reactor import client
from reactor.control import ControlCli
from reactor.launch import LaunchCli
from reactor.optarg import Args
from reactor.action import Action

################################################################################
def new_base(args):
    """
    Common code to get the reactor base object
    ...
    base = reactor.Base(debug=debug, logfmt=logfmt).cfg_load()
    ...
    def __init__(self, cmd):
        ...
        self.args = Args(
            [
                "scope", {
                    "type":"from-set",
                    "set": ["setup", "update|upgrade", "cfg", "apikey", "dse",
                            "launch", "list|ls", "api?key|key", "*"]
                }
            ], [
                "--debug|-d", {
                    "type": "set-add",
        ...
        super(CliReactor, self).__init__(cmd)
...
#                kuali.populate.create_for(dse, args.product, args.service,
#                                          region, shortcode, args.tenant)

################################################################################
class CliCore(CliRoot):
    """core command"""
    ...
    def __init__(self, cmd):
        ...
        super(CliCore, self).__init__(cmd)
...
Where ACTION is one of the defined reactor actions in .pkg/actions.json
