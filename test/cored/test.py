...
try:
    ...
    import io as strio
import reactor.core.db.mxsql
from reactor.core.db.objects import Pipeline, Service, Config, Instance, Policy, Policymatch, Apikey, Build, Group, ObjectExists
import reactor
import reactor.core.abac as abac
from reactor.core import memstate
from reactor.test import *
from reactor import json4human, json4store, json2data
...
class Tester(reactor.Base):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tap = TAP()
        self.dbm = reactor.core.db.mxsql.Master(config={
...
def test_full_stack(schema, base, tester, baseurl):
    ...
    os.environ['REACTOR_URL'] = tester.baseurl

    os.environ['REACTOR_APIKEY'] = master_key.obj['name'] + "." + master_key.obj['secrets'][0]
    rcs_master = client.Session().cfg_load()

    tester.okcmp("Reactor Apikey Create", tester, tester.rcs,
                 [rcs_master.create, "apikey", {
                     "name": "amy-pond"
                 }], {},
                 r"'status': 'created'"
                 )
    tester.okcmp("Reactor Apikey Get", tester, tester.rcs,
                 [rcs_master.get, "apikey", "amy-pond"], {},
                 r"'name': 'amy-pond'")

    pond_apikey = tester.results[1]['name'] + "." + tester.results[1]['secrets'][0]
    os.environ['REACTOR_APIKEY'] = pond_apikey
    rcs_pond = client.Session().cfg_load()
    tester.okcmp("Reactor Client Create (limited)", tester, tester.rcs,
                 [rcs_pond.create, "config", {
                     "name":"tardis-main",
                     "type":"parameter",
                 }], {},
                 r"reactor.client.ClientError: Forbidden"
                 )

    tester.okcmp("Reactor Client Create (master)", tester, tester.rcs,
    ...
    tester.okcmp("Reactor Client Get (limited w/fail)", tester, tester.rcs,
                 [rcs_pond.get, "config", "tardis-main"], {},
                 r"reactor.client.ClientError: Forbidden"
                 )

    tester.okcmp("Reactor Client Get (master)", tester, tester.rcs,
                 [rcs_master.get, "config", "tardis-main"], {},
                 r"'name': 'tardis-main'")

    tester.okcmp("Reactor Client Get List (master)", tester, tester.rcs,
                 [rcs_master.list, "config"], {},
                 r"tardis-main-sub")

    # create a policy allowing amy pond to read any config (sensitive and not)
    tester.okcmp("Reactor Policy Create (pond not sensitive)", tester, tester.rcs,
                 [rcs_master.create, "policy", {
                     "name": "pond-read-configs",
                     "policy": 'token_name=="amy-pond" and sensitive==False'
                 }], {},
                 r"'status': 'created'")
    tester.okcmp("Reactor Policy Create (pond sensitive)", tester, tester.rcs,
                 [rcs_master.create, "policy", {
    ...
    tester.okcmp("Reactor Policymatch Create global", tester, tester.rcs,
    ...
    tester.okcmp("Reactor Client Get (limited w/success)", tester, tester.rcs,
                 [rcs_pond.get, "config", "tardis-main"], {},
                 r"'name': 'tardis-main'",
                 r"'sensitive': 'encrypted'")

    tester.okcmp("Reactor Client Get List (pond sensitive cols w/o)", tester, tester.rcs,
                 [rcs_pond.list, "config"], {'cols': ["name","sensitive"]},
                 r"tardis-main-sub",
                 r"'sensitive': 'encrypted")

    # map the sensitive policy just to the individual items (it is targetted)
    tester.okcmp("Reactor Policymatch Create sensitive", tester, tester.rcs,
                 [rcs_master.create, "policymatch", {
                     "name": "pond-read-sensitive",
                     "policy_id": 102,
                     "actions": 'read',
    ...
    tester.okcmp("Reactor Client Get List (pond)", tester, tester.rcs,
                 [rcs_pond.list, "config"], {},
                 r"tardis-main-sub")

    tester.okcmp("Reactor Client Get List (pond sensitive)", tester, tester.rcs,
                 [rcs_pond.list, "config"], {},
                 r"tardis-main-sub")

    tester.okcmp("Reactor Client Get List (pond sensitive cols)", tester, tester.rcs,
                 [rcs_pond.list, "config"], {'cols': ["name","sensitive"]},
                 r"tardis-main-sub",
                 r"'sensitive': {'config': 'real'}")
    ...
    tester.okcmp("Reactor Policy Drop", tester, tester.rcs,
...
def main():
    ...
    base.term_width = 80
    schema = reactor.core.db.objects.Schema(master=tester.dbm)

    def get_libfiles():
        flist = []
        for (base, folders, files) in os.walk(libdir):
            for f in files:
                if f[-3:] == ".py":
                    if libdir == base:
                        flist.append(f)
    ...
    if args.option == 'unit':
        ...
        tap.lint(libdir + "/..", 'core', exit_on_fail=False)
        return
